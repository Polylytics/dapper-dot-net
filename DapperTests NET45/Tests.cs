...
";
        ...
        public void TestArbitraryMultiMapGrid() {
            // copy and pasted from above for completeness/simplicity
            var createSql = @"
                create table #ReviewBoards (Id int, Name varchar(20), User1Id int, User2Id int, User3Id int, User4Id int, User5Id int, User6Id int, User7Id int, User8Id int, User9Id int)
                create table #Users (Id int, Name varchar(20))
...
";
                    ...
                    var secondSql = "select * from #Users";

                    var grid = connection.QueryMultipleAsync(sql + secondSql).Result;
                    var data = grid.Read(types, mapper);
                    var users = grid.Read<User>();
